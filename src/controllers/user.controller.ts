...
interface UserFilterType extends FilterType {
	search: string
	sortBy: string
	order: string
	limit: number
...
export const addAddress: RequestHandler = (req: UserRequest, res: Response) => {
	try {
		if (!req.user) {
			res.status(404).json({
				error: 'User not found'
...
export const listUser: RequestHandler = async (
	...
	try {
		...
		const users = await User.find(filterArgs).lean()
