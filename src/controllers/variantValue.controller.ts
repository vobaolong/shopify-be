...
export const createValue: RequestHandler = async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      variantValue: savedVariantValue
    })
  } catch (error) {
...
export const updateValue: RequestHandler = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: 'Update variantValue successfully', variantValue })
  } catch (error) {
    res.status(400).json({ error: errorHandler(error as MongoError) })
  }
}
...
export const removeValue: RequestHandler = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: 'Remove variantValue successfully', variantValue })
  } catch (error) {
    res.status(400).json({ error: errorHandler(error as MongoError) })
  }
}
...
export const restoreValue: RequestHandler = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: 'Restore variant Value successfully', variantValue })
  } catch (error) {
    res.status(400).json({ error: errorHandler(error as MongoError) })
  }
}
